...
class GroupSubgroupItemsView extends StatefulWidget {
  ...
  const GroupSubgroupItemsView({super.key,this.isSuperAdmin = false,});
...
Future<void> toggleEdit(String docId, bool isUnlocked) async {
  ...
  if (widget.isSuperAdmin)
    const DataColumn(label: Text("Unlock")), // 🔥 NEW
],
                                rows: docs.map((doc) {
                                    ...
                                    cells: [
                                      ...
                                      DataCell(Text(
                                          d['Opening_Stock']?.toString() ??
                                              "0")),
                                      DataCell(Text(
                                          d['Minimum_Stock']?.toString() ??
                                              "0")),
                                      DataCell(
                                          Text(d['Size']?.toString() ?? "")),
                                      DataCell(
                                          Text(d['Unit']?.toString() ?? "")),
                                      DataCell(
